...
import { CalendarCheck, FileText, MessageSquare, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { StatCard } from "@/components/dashboard/StatCard";
import { useGetPatientMetaQuery } from "@/redux/features/patientDashboard/patientDashboardApi";

const icons = [CalendarCheck, FileText, MessageSquare];

export function PatientOverview() {
  const { data, isLoading, isError, refetch } = useGetPatientMetaQuery();
  ...
  const metrics = [
    ...
    {
      title: "Prescriptions",
      value: String(meta?.prescriptionCount ?? 0),
      helper: "Active prescription history",
    },
    {
      title: "Reviews",
    ...
    return (
      <Card className="border-red-200 bg-red-50">
        <CardContent className="flex flex-col gap-4 p-6 text-red-900 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-semibold">Could not load patient overview</p>
        ...
        <CardContent>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline">
              <Link href="/dashboard/patient/prescriptions">My prescriptions</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
